...
### 1.IO 操作
Groovy 提供了很多[辅助方法](http://groovy-lang.org/gdk.html)，尽管可以使用 Java 来解决，但 Groovy 提供了更方便的方法来处理files, streams, readers, …
...
无论什么原因，`eachLine `语句体内抛出了异常，该方法将确保资源正确关闭. 对于所有 Groovy 添加的所有 I/O 资源方法来说都是适用的

在某些情况下，你更喜欢用` Reader `，也会受益于 Groovy 的自动资源管理. 下面这个例子中，即使发生异常，`Reader `也会关闭. 
```
...
在Java中,获取文件的的byte[]需要很多的代码,但现在，Groovy 将一切都简化了：
```
